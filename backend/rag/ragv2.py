...
class ReasoningRAG:
    ...
    def _generate_reasoning(self, 
        ...
        重点关注相关领域知识，重点关注制造业与电子半导体领域的相关知识，其中制造业核心领域涵盖各类制造模式（如离散制造、流程制造、混合制造）的核心工艺步骤、关键工艺参数优化及工艺瓶颈解决方案等生产工艺与流程内容，包含原材料采购与供应商管理、库存优化策略、生产计划与排程（APS）、仓储与配送效率提升及供应链韧性建设等供应链与物流管理要点，涉及质量控制标准（如 ISO 9001、IATF 16949 等）、质量检测技术与设备、缺陷追溯与分析及行业特定合规要求（如汽车行业的 PPAP、医疗器械的 GMP）等质量管理与合规事项，囊括工业自动化设备（机器人、PLC、传感器等）部署与集成、工业互联网平台搭建、数字孪生技术应用及人工智能在生产优化 / 质量检测中的落地案例等智能制造与技术应用内容，以及生产设备全生命周期管理（TPM）、设备故障预测与健康管理（PHM）、备品备件管理及设备升级与改造方案等设备运维与管理相关信息；电子半导体核心领域则包含芯片架构设计（如 CPU、GPU、MCU 等）、前端设计（RTL 编码、逻辑综合）、后端设计（布局布线、物理验证）及仿真与测试验证方法（如 DFT、ATE）等芯片设计与验证内容，涉及晶圆材料（硅片、化合物半导体）特性与选型、核心制造工艺（光刻、蚀刻、沉积、离子注入、CMP）、工艺节点（如 7nm、5nm、3nm）技术难点与突破及晶圆良率提升策略等晶圆制造与工艺要点，涵盖芯片封装技术（DIP、SOP、QFP、BGA、Chiplet 等）、测试方案设计（功能测试、性能测试、可靠性测试）、封装材料（基板、引线、包封剂）选型及产品可靠性验证标准（如温度循环、湿热测试）等封装测试与可靠性事项，包含核心制造设备（光刻机、刻蚀机、薄膜沉积设备等）的技术参数、国产化进展与供应链风险及关键材料（光刻胶、特种气体、靶材）的性能要求与市场格局等半导体设备与材料内容，还涉及半导体产品相关的国际 / 国内标准（如 JEDEC、ISO）、细分领域（如汽车半导体、AI 芯片、功率半导体）的市场规模、增长驱动因素与竞争格局及环保法规（如 RoHS、WEEE）对生产的影响等行业标准与市场趋势信息 
        """
    ...
    def _synthesize_answer(self, 
        ...
        system_prompt = """
        你是相关领域的专家。基于检索到的信息块，为用户的查询合成一个全面的答案。
...
def multi_hop_generate_answer(query: str, kb_name: str, use_table_format: bool = False, system_prompt: str = "你是一名行业专家。") -> Tuple[str, Dict]:
...
def ask_question_parallel(question: str, kb_name: str = DEFAULT_KB, use_search: bool = True, use_table_format: bool = False, multi_hop: bool = False) -> str:
    ...
    try:
        ...
        if search_background and local_answer:
            system_prompt = "你是一名行业专家，请整合网络搜索和本地知识库提供全面的解答。"
