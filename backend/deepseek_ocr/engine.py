...
class DeepSeekOCREngine:
    ...
    def __init__(self, cache_dir: Optional[str] = None, auto_download: bool = True):
        ...
        with self._lock:
            ...
            logger.info("🔧 DeepSeek OCR Engine initialized")
    ...
    def _ensure_model_downloaded(self):
        ...
        try:
            ...
            if local_model_path.exists():
                ...
                logger.info("   Local model directory not found")
            ...
            try:
                ...
                # 下载模型（如果已存在则跳过）
                model_dir = snapshot_download("deepseek-ai/DeepSeek-OCR", cache_dir=self.cache_dir)
                ...
                try:
                    ...
                    AutoConfig.from_pretrained(self.model_name, trust_remote_code=True, cache_dir=self.cache_dir)
                    logger.info("✅ Model ready (will load on first use)")
        ...
        with self._lock:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning("⚠️  Flash attention not available, using default attention")
    ...
    def parse(
        self, file_path: str, output_path: str, resolution: str = "base", prompt_type: str = "document", **kwargs
    ) -> Dict[str, Any]:
        """
        解析文档或图片
        ...
        try:
            ...
            if self.device == "cuda":
                ...
                gpu_memory_total = torch.cuda.get_device_properties(0).total_memory / 1024**3
                logger.info(
                    f"   GPU 显存: 已分配 {gpu_memory_allocated:.2f}GB / 已保留 {gpu_memory_reserved:.2f}GB / 总计 {gpu_memory_total:.2f}GB"
                )
            ...
            error_msg = str(e)
            logger.error("=" * 80)
            logger.error("❌ RuntimeError 异常详情:")
            logger.error(f"   错误类型: {type(e).__name__}")
                ...
                if "out of memory" in error_msg.lower():
                    ...
                    if self.device == "cuda":
                        try:
                            ...
                            gpu_memory_total = torch.cuda.get_device_properties(0).total_memory / 1024**3
                            logger.error(
                                f"   当前显存: 已分配 {gpu_memory_allocated:.2f}GB / 已保留 {gpu_memory_reserved:.2f}GB / 总计 {gpu_memory_total:.2f}GB"
                            )
                        except Exception:
                            pass
                    logger.error("")
            ...
            # 专门处理除零错误（通常是分辨率太小导致）
            logger.error("=" * 80)
            logger.error("❌ 分辨率配置错误:")
            logger.error("   错误类型: ZeroDivisionError")
            logger.error(f"   错误信息: {e}")
            logger.error("")
            logger.error("🔍 原因分析:")
            ...
            raise RuntimeError(
                f"分辨率 '{resolution}' 对于此图像来说太小。" f"请使用更大的分辨率 (建议: small 或 base)。"
            )
